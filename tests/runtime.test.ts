...
describe("strings", () => {
  it("combines basic classes", () => {
    expect(classigo("btn", "btn--primary", "btn--lg")).toBe(
  ...
  it("filters falsy values", () => {
    expect(classigo("btn", false, null, undefined, "btn--primary")).toBe(
      "btn btn--primary",
    );
  });
  ...
  it("handles conditional with &&", () => {
    expect(
      classigo("btn", true && "btn--active", false && "btn--disabled"),
    ).toBe("btn btn--active");
  });
...
describe("objects", () => {
  it("includes keys with truthy values", () => {
    expect(classigo({ "btn--active": true, "btn--disabled": false })).toBe(
      "btn--active",
    );
  });
...
describe("mixed", () => {
  it("combines strings and objects", () => {
    expect(
      classigo("btn", { "btn--active": true, "btn--disabled": false }),
    ).toBe("btn btn--active");
  });

  it("handles falsy between strings and objects", () => {
    expect(
      classigo("btn", null, { "btn--active": true }, false, "btn--lg"),
  ...
  it("CSS Modules pattern — resolved hashes as strings", () => {
    ...
    expect(classigo(styles.button, styles.primary)).toBe(
      "button_abc123 primary_def456",
    );
  });
  ...
  it("CSS Modules pattern — object shorthand", () => {
    const styles = {
      button: "btn_x1",
      active: "active_y2",
      disabled: "disabled_z3",
    };
    const isActive = true;
    expect(
      classigo(styles.button, {
